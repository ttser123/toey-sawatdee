                    ...
                    <td className="py-3 text-right pr-2">
                      <div className="flex justify-end gap-2 sm:gap-3">
                        {editDraft.id === exp.id ? (
                          ...
                          <>
                            <button onClick={() => setEditDraft({ id: exp.id, label: exp.label, amount: exp.amount.toString(), cat: exp.category, freq: exp.frequency, target: exp.targetMonth || store.viewMonth })} className="text-slate-300 hover:text-indigo-600 transition-all"><span className="material-symbols-outlined text-sm sm:text-base">edit_note</span></button>
                            <button onClick={() => { if (window.confirm(`Are you sure you want to delete expense "${exp.label}"?`)) store.removeExpense(exp.id); }} className="text-slate-300 hover:text-rose-600 transition-all"><span className="material-symbols-outlined text-sm sm:text-base">delete_forever</span></button>
                          </>
                        )}
                      </div>
                    </td>

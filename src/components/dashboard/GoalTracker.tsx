    ...
    <div className="space-y-4 group border-b border-slate-200 pb-6 last:border-0">
      <div className="flex justify-between items-start gap-4">
        <div className="min-w-0 flex-1">
          <span className="text-sm font-bold uppercase truncate block tracking-widest text-slate-800">{goal.label}</span>
          <span className="text-[9px] text-slate-500 font-bold font-mono uppercase tracking-tighter truncate block mt-1">
            TARGET_DATE: {formatMonth(goal.deadline)} // FUND_SOURCE: {goal.assetLabel}
          </span>
        </div>
        <div className="text-right">
          <span className={`text-lg font-black font-mono ${goal.percentage >= 100 ? 'text-emerald-600' : 'text-indigo-600'}`}>
      ...
      <div className="flex items-center gap-2 pt-2">
        <div className="flex-1 flex items-center bg-slate-50 border border-slate-200 rounded-sm overflow-hidden p-1">
          <button
            ...
            className="text-rose-600 hover:bg-rose-50 px-2 py-1 transition-colors shrink-0"
          ...
          <input
            ...
            className="flex-1 bg-transparent outline-none text-[11px] font-bold text-center font-mono text-slate-800 placeholder:text-slate-300 min-w-0"
          />
          <button
            onClick={() => handleTransaction('add')}
            className="text-emerald-600 hover:bg-emerald-50 px-2 py-1 transition-colors shrink-0"
            title="Save Funds"
          >
        ...
        <button 
          ...
          className="text-slate-300 hover:text-rose-600 p-1 transition-all"
          title="Abort Mission"
        >
...
export const GoalTracker = ({ onAddMission }: { onAddMission?: () => void }) => {
  ...
  return (
    <section className="card-blueprint p-6 md:p-8 space-y-6 h-full">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="material-symbols-outlined text-indigo-600 bg-indigo-50 p-2 rounded-sm">target</span>
          <h2 className="text-xl font-bold uppercase tracking-tight text-slate-800">Active_Missions</h2>
        </div>
      </div>

      <div className="space-y-8 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200">
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            {onAddMission && (
              <button
                ...
                className="blueprint-btn-primary px-8 py-3 flex items-center gap-2"
              >
      ...
      <div className="pt-4 border-t border-slate-200">
        <p className="text-[9px] text-slate-400 font-mono uppercase tracking-widest text-center italic">
